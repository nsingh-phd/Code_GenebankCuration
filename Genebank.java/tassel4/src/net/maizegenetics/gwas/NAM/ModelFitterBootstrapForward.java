...
//		System.out.println(bestSnp.pos + ", " + bestSnp.allele + ", " + bestSnp.F + ", " + bestSnp.p);
	...
	private void buildPopulationList() {
		...
		while (mit.hasNext()) {
			...
			popSubsampleSize[pcount] = (int) Math.ceil(m * files.samplePercent);
	...
	private void getSubsample() {
		if (!files.resample) {
			int ntaxa = residuals.length;
			totalSubsampleSize = ntaxa;
			subsample = new int[ntaxa];
			for (int i = 0; i < ntaxa; i++) subsample[i] = i;
		} else {
			int start = 0;
