...
public class TagsToSNPsNoAnchor {
	 ...
	 * @param minAlleleFreq
	 * @param isDHpopulation
	 *            , asking if population is double haploid. If so, will reject
	 *            any SNPs that are heterozygous,
	 */
	public TagsToSNPsNoAnchor(TagsByTaxa theTBT, String outHapMap,
			int divergence, double maxMissingData, double minAlleleFreq,
			double maxHet, boolean isDHpopulation, boolean isBiparental,
			boolean callHets, double pVal) {

		System.gc();
		// do some checks
		if (maxMissingData > 1 || maxMissingData < 0) {
			System.out
					.println("Max Missing Data (maxMissingData) must be between 0 and 1.  Entered Value: "
							+ maxMissingData);
			System.exit(0);
		}
		if (minAlleleFreq > 0.5 || minAlleleFreq < 0) {
			System.out
					.println("Minimum Allele Frequency (minAlleleFreq) must be between 0 and 0.5.  Entered Value: "
							+ minAlleleFreq);
			System.exit(0);
		}
		...
		System.out
				.println("Comp	Tag1	Tag2	Divergence	Tag1Cnt1	Tag2Cnt2	Cnt11	exp11	Ratio11OE	P");

		int pauses = 0;
		ArrayList<filteredSnpPair> filSnpList = new ArrayList<filteredSnpPair>(
				0);

		boolean[] tested = new boolean[theTBT.getTagCount()];

		try {
			BufferedWriter bw = new BufferedWriter(new FileWriter(outHapMap),
					65536);
			...
			for (int i = 0; (i < theTBT.getTagCount()) && (nSNPs < maxSize); i++) {
				...
				OpenBitSet bitDistA = theTBT.getTaxaReadBitsForTag(i); // the
																		// taxa
																		// with
																		// tagA

				// if(bitDistA.cardinality() < nTaxa*0.3*(1-maxMissingData))
				// continue; // skip the rare tags, use tags that show up at
				// reasonable frequency (major allele)
				// if(bitDistA.cardinality() < 8) continue; // skip the rare
				// tags, use tags that show up at reasonable frequency (major
				// allele)
				if (reference.get(i))
					continue; // check if this tag has been used as a reference,
								// skip if so
				reference.set(i); // set this tags a being a reference

				TreeMap<Integer, Integer> al = theTMF
						.findMatchesWithIntLengthWords(tagA, divergence, false); // TreeMap
																						// each
																					// pair
				if (al.size() < 2)
					continue; // skip stuff that only has one match (self)

				int refCnt = 0;
				for (Entry<Integer, Integer> ht : al.entrySet()) {
					if (reference.get(ht.getKey()))
						refCnt++;
				}
				if (refCnt > 1)
					continue; // skip as there was already a reference tag for
								// this TreeMap
				...
				for (Entry<Integer, Integer> ht : al.entrySet()) {
					...
					// if(ht.getKey()<refTagIdx){refTagIdx = ht.getKey();} //set
					// the first tag as the reference
				}
				// if(refTagIdx<i) continue; //skip tags that were previously
				// tested
				...
				for (int idx = 0; idx < tags.get(0).length; idx++) {
					...
					byte a2 = tags.get(0)[idx]; // this will fail on tri-allelic
												// sites
					...
					if (a1 == a2)
						continue; // skip over non-variable sites

					OpenBitSet bitDistB = theTBT.getTaxaReadBitsForTag(0);
					for (int z = 0; z < bitDistB.size(); z++) {
						bitDistB.fastClear(z);
					}

					for (int t = 0; t < tags.size(); t++) {
						if (tags.get(t)[idx] == a1) {
							OpenBitSet bitDistTmp = theTBT
									.getTaxaReadBitsForTag(hitIdx[t]);
							for (int b = 0; b < bitDistA.size(); b++) {
								if (bitDistTmp.fastGet(b))
									bitDistA.fastSet(b);
							}
						}
					...
					int cnt11 = (int) OpenBitSet.intersectionCount(bitDistA,
							bitDistB); // number of lines with both tags
					int cntWTag = (int) OpenBitSet.unionCount(bitDistA,
							bitDistB); // number of lines with one of the two
										// tags
					int cnt00 = cntTaxa - cntWTag;

					double propPresent = (double) cntWTag / (double) cntTaxa;
					if (propPresent < (1 - maxMissingData))
						continue; // skip if missing too much data

					// filter for heterozygousity
					if (isDHpopulation && cnt11 > 0)
						continue; // skip if there are any heterozygous calls
					// if(cnt11>10) continue; // filter that the two tags don't
					// show up in the same individuals

					double percentHet = (double) cnt11 / (double) cntWTag; // TODO
																				// (currently)
					if (percentHet > maxHet)
						continue; // skip if heterozygousity is > maxHet

					// check if the snp pair has segregation distortion, this
					// basically does the same thing as the minor allele freq

					// if(isBiparental && (Math.abs(percent10-0.5)>0.3 ||
					// Math.abs(percent01-0.5)>0.3)) continue; // skip if one
					// allele is >80% or <20%

					double freqA = (double) cnt10 / (double) cntWTag;
					double freqB = (double) cnt01 / (double) cntWTag;
					if (Math.min(freqA, freqB) < minAlleleFreq)
						continue;
					...
					if (cnt11 > a * b * (double) cntTaxa * 0.6)
						continue; // skip stuff that has too much hets

					// filter for two alleles are not associated (inbred)
					// double
					// exp11=(double)cnt01*(double)cnt10/(double)theTBT.getTaxaCount();
					...
					nSNPs++;
					// String snps[] =
					// makeSNPCalls(BaseEncoder.getSequenceFromLong(tagA),
					// BaseEncoder.getSequenceFromLong(tagB));
					...
					// filteredSnpPair fsp = new filteredSnpPair(snps[1],
					// snps[0], i, hitIndex, div, cnt10, cnt01, cnt11, exp11,
					// relExp11, p, bitDistA, bitDistB, callHets);
					// fsp.swap();
					// filSnpList.add(fsp);

					// "rs	alleles	chrom	pos	strand	assembly	center	protLSID	assayLSID	panelLSID	QCcode	"
					// TODO need to check if this shoudl be getTagLength(i)-1.
					// That was outputting 63 bp strings
					bw.write(BaseEncoder.getSequenceFromLong(tagA).substring(0,
							theTBT.getTagLength(i))
							+ "\t"
							+ snps[0]
							+ "\t"
							+ "0"
							...
							+ cnt10
							+ "\t"
							+ cnt01 + "\t" + cnt11 + "\t" + propPresent + "\t");
					bw.write(bitsToPseudoSeq(theTBT.getTaxaCount(), bitDistA,
							bitDistB, snps[0], callHets, numericOutput) + "\n");
					...
					// System.out.printf("con %d %d %d %d %d %d %g %g %g ",i,
					// hitIndex, div, cnt10, cnt01, cnt11, exp11, relExp11, p);
					// System.out.print("\n");
					System.out.printf("con %d %d %d %d %d %d %g ", i, hitIndex,
							div, cnt10, cnt01, cnt11, p);
					System.out.println();

					for (int t = 0; t < tags.size(); t++) {
						System.out.print(BaseEncoder.getSequenceFromLong(theTBT
								.getTag(hitIdx[t])) + "\t");
						OpenBitSet bitDist = theTBT
								.getTaxaReadBitsForTag(hitIdx[t]);
						for (int bt = 0; bt < bitDist.size(); bt++) {
							System.out.print(bitDist.getBit(bt) + "\t");
						}
						System.out.println();
					}
					System.out
							.println("\t\t\t\t\t\t\t\t\t"
									+ bitsToPseudoSeq(theTBT.getTaxaCount(),
											bitDistA, bitDistB, snps[0],
											callHets, numericOutput));
					System.out.println();
	...
	class filteredSnpPair implements Comparable<filteredSnpPair> {
		...
		filteredSnpPair(String sequence, String snp, int queryIndex,
				int hitIndex, int div, int cCnt, int hCnt, int cnt11,
				double exp11, double relExp11, double p, OpenBitSet cBitDist,
				OpenBitSet hBitDist, boolean callHets) {
		...
		public String mkStr(String name, int chr, int posi, int taxaCount,
				boolean callHets, boolean numericOut) {
			StringBuilder sb = new StringBuilder();
			sb.append(name).append("\t").append(snp).append("\t").append(chr)
					.append("\t").append(posi)
					.append("\t+\tNA\tSWGDiv\tGBS\tSWGV1\tSWGPop\tQC+\t");
			sb.append(bitsToPseudoSeq(taxaCount, cBitDist, hBitDist, snp,
					callHets, numericOut));
	...
	public static ArrayList<filteredSnpPair> collapseSeqError(
			TagsByTaxa theTBT, ArrayList<filteredSnpPair> filSnpList) {
		...
		while (i < filSnpList.size()) {
			...
			TreeMap<Integer, Integer> al = theTMF
					.findMatchesWithIntLengthWords(tagA, 2, false);
	...
	public static String bitsToPseudoSeq(int numTaxa, OpenBitSet t1,
			OpenBitSet t2, String snp, boolean callHets, boolean numericOut) {
		StringBuilder sb = new StringBuilder();
		// String[] allele={"A","C","R"};
	...
	public static void main(String[] args) {
		...
		TagsByTaxa theTBT = new TagsByTaxaBitFileMap(
				"/homes/trife/gbs/Shan-T4/tbtMerge_Shan-T4_20140409.bin");

		String outHapMap = "/homes/trife/gbs/Shan-T4/test.hap";
